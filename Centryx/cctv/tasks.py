...
def get_brand_status(brand_name):
    ...
    try:
        ...
        if brand_name == 'dahua':
            ...
            # Production: do not log device lists
            total = len(devices)
        ...
        # -----------------------
            ...
            for device in devices:
                if device.get('status', '').lower() == 'online':
                    ...
                    offline += 1
        # -----------------------
        # Other brands can be added here
        # -----------------------
    except Exception as e:
        logger.error(f"Error updating statistics for brand {brand_name}: {e}")
    return {
        'brand': brand_name,
        'total': total,
        'online': online,
...
def populate_live_urls_cache(
        self,
        cache_key: str,
        ttl: Optional[int] = None,
        rate_delay: float = 0.21,
        lock_ttl: int = 60 * 5):
    """
    ...
    # Try to acquire lock; cache.add returns True if key was set (i.e., lock
    # acquired)
    got_lock = False
    try:
        got_lock = cache.add(lock_key, "1", lock_ttl)
    except Exception as e:
        # If cache backend doesn't support add or fails, log and continue
        # (best-effort)
        logger.exception("Failed to acquire lock via cache.add: %s", e)

    if not got_lock:
        logger.info(
            "populate_live_urls_cache: another worker is already refreshing cache %s",
            cache_key)
        return {"skipped": True}
    ...
    try:
        ...
        # Build aggregate keyed by camera id to ensure uniqueness even if
        # locations repeat.
        result = {}
        ...
        # Iterate cameras and call serializer per-object to give us control
        # over pacing
        for camera in cameras:
            try:
                # Wait for a rate slot before calling external clients
                _acquire_rate_token(limit_per_second=max(
                    1, int(1.0 / max(0.0001, rate_delay))))

                serializer = CameraLiveUrlSerializer(camera)
                data = serializer.data
                location = data.get("location") or "Unknown Location"
                cid = str(camera.id)
                result[cid] = {"camera_id": cid, "location": location,
                               "live_url": data.get("live_url")}
            except Exception:
                # Don't fail the whole task for a single camera; log and
                # continue
                logger.exception(
                    "Error fetching live URL for camera %s", getattr(
                        camera, "identifier", "?"))
